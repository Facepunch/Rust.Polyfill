...
namespace Facepunch.Extend
{
    public static class StringExtensions
    {
        ...
        private static char[] spaceOrQuote = new char[] { ' ', '"' };
        ...
        {
            // Would use the pool but its not threadsafe yet
        ...
        public static string[] SplitQuotesStrings( this string input, int maxCount = int.MaxValue )
        {
            ...
            var match = regexSplitQuotes.Match( input );

            for ( int i = 0; i < maxCount && match.Success; i++ )
            {
                var split = match.Value;
                split = split.Trim( spaceOrQuote );
                split = split.Replace( "&qute;", "\"" );
                list.Add( split );
        ...
        public static long ToLong( this string str, long Default = 0 )
        {
            ...
            return num <= min ? long.MinValue : num >= max ? long.MaxValue : (long)num;
        }
        ...
        public static bool ToBool( this string str )
        {
            ...
            if ( str == "False" ) return false; // fast path for ToString'd value
            if ( str == "True" ) return true; // fast path for ToString'd value
