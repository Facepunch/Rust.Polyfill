...
namespace Facepunch.Extend
{
    public static class NumberExtensions
    {
        ...
        public static string FormatSecondsLong( this long s )
        {
            ...
            if ( m < 60 ) return string.Format( "{1} minutes", s % 60, m, h, d, w ); // 5m3s
            if ( h < 48 ) return string.Format( "{2} hours", s % 60, m % 60, h, d, w ); // 6h40m34h
            if ( d < 2 ) return string.Format( "{3} days, {2} hours", s % 60, m % 60, h % 24, d % 7, w ); // 5d15h15m10s

            return string.Format( "{3} days", s % 60, m % 60, h % 24, d, w );
        }
        ...
        {
            return $"{num:F1}m";//todo: expose number of decimal points?
        }
    }
}
