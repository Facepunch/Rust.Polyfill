...
namespace Facepunch
{
    public static class CommandLine
    {
        ...
        /// <summary>
        /// Returns the full command line with some values omitted, reconstructed from System.Environment.GetCommandLineArgs
        /// </summary>
        public static string FullSafe
        {
            get { Initalize(); return safecommandline; }
        }
        ...
        private static void Initalize()
        {
            ...
            if ( commandline == "" )
            {
                ...
                foreach ( string arg in args )
                {
                    ...
                    if (arg.Contains("password", StringComparison.InvariantCultureIgnoreCase)) skipNext = true;
                    if (arg.Contains("anticheatid", StringComparison.InvariantCultureIgnoreCase)) skipNext = true;
                    if (arg.Contains("anticheatkey", StringComparison.InvariantCultureIgnoreCase)) skipNext = true;
                    if (arg.Contains("azure_client_secret", StringComparison.InvariantCultureIgnoreCase)) skipNext = true;
                    if (arg.Contains("azure_client_id", StringComparison.InvariantCultureIgnoreCase)) skipNext = true;
                    if (arg.Contains("azure_tenant_id", StringComparison.InvariantCultureIgnoreCase)) skipNext = true;
#if UNITY_EDITOR
                    if (arg.Contains("accessToken", StringComparison.InvariantCultureIgnoreCase)) skipNext = true;
                    if (arg.Contains("hubSessionId", StringComparison.InvariantCultureIgnoreCase)) skipNext = true;
#endif
                }
            }
